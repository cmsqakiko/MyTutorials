...
var data = {
  ...
  tracks: [createTrack("gold", note(880)),
           createTrack("gold", note(659)),
           createTrack("gold", note(587)),
           createTrack("gold", note(523)),
           createTrack("gold", note(440)),
           createTrack("dodgerblue", kick)]
};
...
(function setupButtonClicking() {
  ...
  document.getElementById("screen").addEventListener("click", function(e) {
    ...
    // Create the audio context the first time a box is clicked
    if (audio === undefined) audio = new AudioContext();
